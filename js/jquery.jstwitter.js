$(function() {		
	...
	JQTWEET = {
	    ...
	    search: '%23jeremiahsice', //leave this blank if you want to show user's tweet
	    user: '', //username
	...
	JQTWEET2 = {
	    ...
	    // for multiple hashtags, you can separate the hashtag with OR, eg:
	    // hash: '%23jquery OR %23css'			    
	    search: '%23bucadibeppo', //leave this blank if you want to show user's tweet
	    user: '', //username
	    numTweets: 10, //number of tweets
	    appendTo: '#jstwitter2',
	    useGridalicious: true,
	    ...
	    // for multiple hashtags, you can separate the hashtag with OR, eg:
	    // hash: '%23jquery OR %23css'			    
	    search: '%23pancheros', //leave this blank if you want to show user's tweet
	    user: '', //username
	    numTweets: 10, //number of tweets
	    appendTo: '#jstwitter3',
	    useGridalicious: true,
	    ...
	    loadTweets: function() {
	        ...
	        $.ajax({
	            ...
	            success: function(data, textStatus, xhr) {
		            ...
		            if (data.httpstatus == 200) {
		            	if (JQTWEET3.search) data = data.statuses;

	                var text, name, img;	         
	                	                
	                try {
	                  // append tweets into page
	                  for (var i = 0; i < JQTWEET3.numTweets; i++) {		
	                    ...
	                    $(JQTWEET3.appendTo).append( JQTWEET3.template.replace('{TEXT}', JQTWEET3.ify.clean(data[i].text) )
	                        .replace('{USER}', data[i].user.screen_name)
	                        .replace('{IMG}', img)                                
	                        .replace('{AGO}', JQTWEET3.timeAgo(data[i].created_at) )
	                        .replace('{URL}', url )			                            
	                        );
	                  }
	                  ...
	                  //item is less than item count
		                ...
		                if (JQTWEET3.useGridalicious) {                
			                //run grid-a-licious
											$(JQTWEET3.appendTo).gridalicious({
												gutter: 13, 
												width: 200, 
												animate: true
											});	                   
